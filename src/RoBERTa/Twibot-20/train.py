...
split = [[], [], []]
path0 = Path('datasets/Twibot-20') #datasets/Twibot-20
...
class Twibot20Dataset(Dataset):
    ...
    def __init__(self, name, device='cuda:6'):
        self.device = torch.device(device)
        path1 = Path("src/BotRGCN/twibot_20/processed_data")
        path2 = Path("src/RoBERTa/Twibot-20")
        
        tweets_tensor = torch.load(path1 / 'tweets_tensor.pt').to(self.device)
        des_tensor = torch.load(path1 / 'des_tensor.pt').to(self.device)
