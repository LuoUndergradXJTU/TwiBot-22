...
split = [[], [], []]
path0 = Path('datasets/cresci-2017')
...
class CresciDataset(Dataset):
    def __init__(self, name, device='cuda:4'):
        self.device = torch.device(device)
        # path1 = Path("src/botrgcn_new/cresci_17/processed_data")
        path2 = Path("src/RoBERTa/cresci-2017")
        path3 = Path('src/GCN_GAT/GCN_GAT/data_17')
        
        tweets_tensor = torch.load(path3 / 'tweets_tensor.pt').to(self.device)
        des_tensor = torch.load(path2 / 'des_tensor.pt').to(self.device)
        label = torch.load(path2 / 'label_list.pt').to(self.device)
...
if __name__ == '__main__':
    train_dataset = CresciDataset('train')
    val_dataset = CresciDataset('val')
    test_dataset = CresciDataset('test')
