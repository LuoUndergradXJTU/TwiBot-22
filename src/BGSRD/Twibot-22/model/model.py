...
class BERT_GAT(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        self.cls_feats = batch.x[:,:768].cuda().half()
        self.edge_index = batch.edge_index.cuda()
        input_ids = batch.x[:batch.batch_size,768:768+batch.input_ids_dim].cuda().long()
        attention_mask = batch.x[:batch.batch_size,-batch.input_ids_dim:].cuda().long()
        
        cls_feats = self.bert_model(input_ids, attention_mask)[0][:, 0]

        # cls_feats = self.cls_feats[:batch.batch_size]
        ...
        self.cls_feats[:batch.batch_size] = cls_feats.half()

        
        gcn_logit = self.gcn(self.cls_feats,self.edge_index)[:batch.batch_size]
    ...
    def training_step(self, batch, batch_idx):
        ...
        y_true = batch.y.cuda()
        y_true = y_true[:batch.batch_size]
