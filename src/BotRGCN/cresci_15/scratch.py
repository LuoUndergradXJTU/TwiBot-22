...
# to do: convert this to a config file and print the output to a log file
...
@torch.no_grad()
def validation(epoch, name, model, loss_fn, loader, dataframe):
    model.eval()
    all_label = []
    all_pred = []
    ave_loss = 0.0
    ...
    # Add one row into dataframe
    dataframe = dataframe.append({'epoch': epoch,
                                    'loss': ave_loss,
                                    'acc': metrics['acc'],
                                    ...
                                    'pr-auc': metrics['pr-auc']}, ignore_index=True)
    if visible:
        print(plog)
    return metrics
...
def train():
    ...
    cnt = 0
    val_dataframe = pd.DataFrame(columns=['epoch', 'loss', 'acc', 'f1-score', 'precision', 'recall', 'mcc', 'roc-auc', 'pr-auc'])
    for epoch in pbar:
        val_metrics = forward_one_epoch(epoch, model, optimizer, loss_fn, train_loader, val_loader, val_dataframe)
