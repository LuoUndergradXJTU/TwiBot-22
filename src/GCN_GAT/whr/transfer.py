...
def forward_one_epoch(model, optimizer, loss_fn, train_loader, test_loader):
    ...
    for batch in train_loader:
        ...
        optimizer.step()
    ave_loss /= cnt
    test_loss, test_acc = validation(model, loss_fn, test_loader)
    return ave_loss, test_loss, accuracy_score(labels, preds), test_acc
...
def validation(model, loss_fn, loader):
    ...
    for batch in loader:
        ...
        cnt += n_batch
    ave_loss /= cnt
    return ave_loss, accuracy_score(labels, preds)
...
def train(train_id, test_id):
    ...
    cnt = 0
    for _ in pbar:
        train_loss, test_loss, train_acc, test_acc = forward_one_epoch(model,
                                                                       optimizer,
                                                                       loss_fn,
                                                                       train_loader,
                                                                       test_loader)
