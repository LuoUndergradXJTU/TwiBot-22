...
from dataset import hetero_graph_vectorize, homo_graph_vectorize, homo_graph_vectorize_only_user, df_to_mask
...
def generate_hetero_graph_loader(num_layers, num_neighbor_users, num_neighbor_tweets, batch_size, include_node_feature, dataset, server_id):
    ...
    graph, uid_to_user_index, tid_to_tweet_index, train_uid_with_label, valid_uid_with_label, test_uid_with_label,user_index_to_uid,tweet_index_to_tid = hetero_graph_vectorize(include_node_feature, dataset, server_id)
    
    train_mask = df_to_mask(train_uid_with_label, uid_to_user_index, "train")
    valid_mask = df_to_mask(valid_uid_with_label, uid_to_user_index, "val")
    test_mask = df_to_mask(test_uid_with_label, uid_to_user_index, "test")
    
    train_loader = HGTLoader(graph, num_neighbors={"user": [num_neighbor_users] * num_layers, "tweet": [num_neighbor_tweets] * num_layers}, input_nodes=('user',train_mask), batch_size=batch_size)
    valid_loader = HGTLoader(graph, num_neighbors={"user": [num_neighbor_users] * num_layers, "tweet": [num_neighbor_tweets] * num_layers}, input_nodes=('user',valid_mask), batch_size=batch_size)
    test_loader = HGTLoader(graph, num_neighbors={"user": [num_neighbor_users] * num_layers, "tweet": [num_neighbor_tweets] * num_layers}, input_nodes=('user',test_mask), batch_size=batch_size)
