...
with open(edge_dir) as csvfile:
    ...
    for row in spamreader:
        index += 1
        if row[1] == 'post':
            if row[0] not in data['uid_to_user_index']:
                continue
            user_index_1 = data['uid_to_user_index'][row[0]]
            if num_of_tweet[user_index_1] < 20:
                num_of_tweet[user_index_1] += 1
                tid_to_tweet_index[row[2]] = user_index_1

        if row[0] not in data['uid_to_user_index'] or row[2] not in data['uid_to_user_index']:
            continue
        user_index_1 = data['uid_to_user_index'][row[0]]
        user_index_2 = data['uid_to_user_index'][row[2]]
