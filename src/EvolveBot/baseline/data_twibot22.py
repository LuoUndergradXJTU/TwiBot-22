...
twibot_22 = dataset_dir / 'Twibot-22'
user_dir = twibot_22 / 'user.json'
label = pd.read_csv(twibot_22 / 'label.csv')
split = pd.read_csv(twibot_22 / 'split.csv')

node_num = 800000
stop_time = 1651332380
        ...
        if time_seen is not None:
            if x['created_at'].find('L')!=-1:
                ...
                real_time = stop_time - time.mktime(
                    time.strptime(time_seen, "%Y-%m-%d %H:%M:%S+00:00")
                )
        else:
            real_time = 0
        r[i][3] = real_time
        r[i][10] = (0 if real_time == 0 else r[i][0] / real_time)
        if find_num % 50000==0 and find_num!=0:
            print(find_num)
        if find_num == node_num:
            break
tokenizer_berttiny = BertTokenizerFast.from_pretrained("prajjwal1/bert-tiny", model_max_length=512)
...
for i in range(node_num):
    ...
    r[i, 9] = 0 if len(followers_of_following[i]) == 0 or statistics.median(followers_of_following[i])==0 else r[i, 0] / statistics.median(followers_of_following[i])

np.save('uid_to_user_index.npy', uid_to_user_index)
np.save('tid_to_tweet_index.npy', tid_to_tweet_index)
np.save('followers_of_following.npy', followers_of_following)
np.save('num_of_tweet.npy', num_of_tweet)
np.save('r.npy', r)
np.save('r_help.npy', r_help)
...
for i in range(9):
    tweet_dir = twibot_22 / f'tweet_{i}.json'
    print(tweet_dir)
    with open(tweet_dir, "r") as f:
        obj = ijson.items(f, "item")
        index = 0
        for x in obj:
            index += 1
