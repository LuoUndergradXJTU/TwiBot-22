...
def preprocessing(node, edge, dataset):
    X = []
    id_map = dict()
    num_user = 0
    ind_first = True
    ...
    #                 age_list.append(0.0)
    #                 continue
    #             born_date = datetime(int(time_list[-1]), MONTH_DICT[time_list[1]], int(time_list[2]),
    #                                  int(time_list[3].split(':')[0]), int(time_list[3].split(':')[1]),
    #                                  int(time_list[3].split(':')[2]))
    ...
    Feature_matrix.to_csv('feature_matrix_' + dataset + '_NoGraph.csv')
