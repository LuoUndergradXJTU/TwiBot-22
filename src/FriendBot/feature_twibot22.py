...
def preprocess(user, tid, author, edge, id_tweet):
    X = []
    id_map = dict()
    tid_map = dict()
    num_user = len(user)
    ...
    for i in range(len(tid)):
        tid_map[tid[i]] = i
    following = edge.values[edge['relation'].values == 'following']
    mentioned = edge.values[edge['relation'].values == 'mentioned']
    retweeted = edge.values[edge['relation'].values == 'retweeted']
    replied = edge.values[edge['relation'].values == 'replied_to']
    graph = nx.Graph()
    print("Building graph...", flush=True)
    graph.add_nodes_from(list(range(num_user)))
    for i in tqdm(range(following.shape[0])):
        graph.add_edge(id_map[following[i, 0]], id_map[following[i, 2]])
    for i in tqdm(range(mentioned.shape[0])):
        graph.add_edge(id_map['u' + str(author[tid_map[mentioned[i, 0]]])], id_map[mentioned[i, 2]])
    for i in tqdm(range(retweeted.shape[0])):
        graph.add_edge(id_map['u' + str(author[tid_map[retweeted[i, 0]]])], id_map['u' + str(author[tid_map[retweeted[i, 2]]])])
    for i in tqdm(range(replied.shape[0])):
        graph.add_edge(id_map['u' + str(author[tid_map[replied[i, 0]]])], id_map['u' + str(author[tid_map[replied[i, 2]]])])

    ego_networks = []
    print("Building ego networks...", flush=True)
    for i in tqdm(range(num_user)):
        ego_node = id_map[user[i]['id']]
        G = nx.Graph()
        G.add_node(ego_node)
    ...
    for i in tqdm(range(num_user)):
        if user[i]['username'] is not None:
            screen_name_length.append(len(user[i]['username']))
        else:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['profile_image_url'] is not None:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['username'] is not None:
            entropy_screen_name.append(str_entropy(user[i]['username']))
        else:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['location'] is not None:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['public_metrics']['tweet_count'] is not None:
            total_tweets.append(user[i]['public_metrics']['tweet_count'])
        else:
    ...
    for i in tqdm(range(num_user)):
        ...
        for j in list(ego_networks[i].nodes):
            if user[j]['profile_image_url'] is not None:
                image_count += 1.0
    ...
    for i in tqdm(range(num_user)):
        tweet_count_list = []
        for j in list(ego_networks[i].nodes):
            if user[j]['public_metrics']['tweet_count'] is not None:
                tweet_count_list.append(user[j]['public_metrics']['tweet_count'])
    ...
    for i in tqdm(range(num_user)):
        age_list = []
        for j in list(ego_networks[i].nodes):
            if user[j]['created_at'] is not None:
                time_list = user[j]['created_at'].split(' ')
                # print(time_list)
                # time_list.pop()
                if len(time_list) == 1:
                    age_list.append(0.0)
                    continue
                born_date = datetime(int(time_list[0].split('-')[0]), int(time_list[0].split('-')[1]),
                                     int(time_list[0].split('-')[2]), int(time_list[1].split('+')[0].split(':')[0]),
                                     int(time_list[1].split('+')[0].split(':')[1]),
    ...
    for i in tqdm(range(num_user)):
        description_count = 0
        for j in list(ego_networks[i].nodes):
            if user[j]['description'] is not None:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['public_metrics']['following_count'] is not None:
            number_of_friends.append(user[i]['public_metrics']['following_count'])
        else:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['public_metrics']['followers_count'] is not None:
            number_of_followers.append(user[i]['public_metrics']['followers_count'])
        else:
    ...
    for i in tqdm(range(num_user)):
        ...
        for j in ego_networks[i].nodes():
            if user[j]['public_metrics']['followers_count'] is not None:
                median_followers.append(user[j]['public_metrics']['followers_count'])
    ...
    for i in tqdm(range(num_user)):
        median_friends = []
        for j in ego_networks[i].nodes():
            if user[j]['public_metrics']['following_count'] is not None:
                median_friends.append(user[j]['public_metrics']['following_count'])
    ...
    #         tweet_object = ''
    #     user_list = []
    #     if user[i]['name'] is not None:
    #         user_list.append(user[i]['name'])
    #     if user[i]['username'] is not None:
    #         user_list.append(user[i]['username'])
    #     if user[i]['description'] is not None:
    #         user_list.append(user[i]['description'])
    #     if len(user_list) == 0:
    #         user_object = ''
    #     else:
    ...
    for i in tqdm(range(num_user)):
        if user[i]['description'] is not None:
            if re.match('[Bb][Oo][Tt]', str(user[i]['description'])) is not None:
                bot_reference.append(1)
                continue
        if user[i]['name'] is not None:
            if re.match('[Bb][Oo][Tt]', str(user[i]['name'])) is not None:
                bot_reference.append(1)
                continue
        if user[i]['username'] is not None:
            if re.match('[Bb][Oo][Tt]', str(user[i]['username'])) is not None:
                bot_reference.append(1)
                continue
    ...
    #     if user[i]['name'] is not None:
    #         languages_dict[langid.classify(user[i]['name'])[0]] = 0
    #     if user[i]['username'] is not None:
    #         languages_dict[langid.classify(user[i]['username'])[0]] = 0
    #     if user[i]['description'] is not None:
    #         languages_dict[langid.classify(user[i]['description'])[0]] = 0
    ...
    #         if user[j]['name'] is not None:
    #             languages_dict[langid.classify(user[j]['name'])[0]] = 0
    #         if user[j]['username'] is not None:
    #             languages_dict[langid.classify(user[j]['username'])[0]] = 0
    #         if user[j]['description'] is not None:
    #             languages_dict[langid.classify(user[j]['description'])[0]] = 0
    ...
    for i in tqdm(range(num_user)):
        if user[i]['created_at'] is not None:
            time_list = user[i]['created_at'].split(' ')
            # time_list.pop()
            if len(time_list) == 1:
        ...
        for j in ego_networks[i].nodes():
            ...
            for k in range(len(tweet_map[j])):
                if tweet_map[j][k][0:4] == 'RT @':
                    retweet_count += 1
            if user[j]['public_metrics']['followers_count'] is not None:
                follower_count = user[j]['public_metrics']['followers_count']
            if user[j]['public_metrics']['following_count'] is not None:
                following_count = user[j]['public_metrics']['following_count']
    ...
    Feature_matrix.to_csv('feature_matrix_Twibot-22.csv')
