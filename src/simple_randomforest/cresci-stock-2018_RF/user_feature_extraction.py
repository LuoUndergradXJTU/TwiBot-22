...
def get_user_list():
    data = pd.read_csv('/data2/whr/czl/TwiBot22-baselines/datasets/cresci-stock-2018/label.csv')
    length = len(data)
...
if __name__ == '__main__':
    start = time.time()
    filepath = './u_feature.csv'
    user_list = get_user_list()
    node = open('/data2/whr/czl/TwiBot22-baselines/datasets/cresci-stock-2018/node.json', "rb")
    f = json.load(node)
    for user in f:
        if user['id'][0] == 'u':
