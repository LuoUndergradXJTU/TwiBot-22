...
def get_user_list():
    data = pd.read_csv('/data2/whr/czl/TwiBot22-baselines/datasets/gilani-2017/label.csv')
    all_user = data.loc[0:len(data)-1]
...
if __name__ == '__main__':
    start = time.time()
    filepath = './users_feature.csv'
    user_list = get_user_list()
    node = open('/data2/whr/czl/TwiBot22-baselines/datasets/gilani-2017/node.json', "rb")
    f = json.load(node)
    print('json_len:',len(f))
    for user in f:
        if user['id'][0] == 'u':
