...
def get_user_list():
    data = pd.read_csv('/data2/whr/czl/TwiBot22-baselines/datasets/pronbots-2019/label.csv')
    length = len(data)
...
if __name__ == '__main__':
    ...
    node = open('/data2/whr/czl/TwiBot22-baselines/datasets/pronbots-2019/node.json', "rb")
    f = json.load(node)
    for user in f:
        if user['id'][0] == 'u':
