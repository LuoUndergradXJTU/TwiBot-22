...
with open(os.path.join(path4, 'node.json'), 'r', encoding='UTF-8') as f:
    node1 = json.load(f)
# node1 = pandas.read_json(os.path.join(path1, 'node.json'))
label1 = pd.read_csv(os.path.join(path4, 'label.csv'))
split1 = pd.read_csv(os.path.join(path4, 'split.csv'))
...
np.savetxt('featurem2018.csv', feature, delimiter = ',')
