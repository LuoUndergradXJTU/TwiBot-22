...
with open(os.path.join(path5, 'node.json'), 'r', encoding='UTF-8') as f:
    node1 = json.load(f)
# node1 = pandas.read_json(os.path.join(path1, 'node.json'))
label1 = pd.read_csv(os.path.join(path5, 'label.csv'))
split1 = pd.read_csv(os.path.join(path5, 'split.csv'))
...
np.savetxt('featureg2017.csv', feature, delimiter = ',')