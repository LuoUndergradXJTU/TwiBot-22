...
with open(os.path.join(path7, 'node.json'), 'r', encoding='UTF-8') as f:
    node1 = json.load(f)
# node1 = pandas.read_json(os.path.join(path1, 'node.json'))
label1 = pd.read_csv(os.path.join(path7, 'label.csv'))
split1 = pd.read_csv(os.path.join(path7, 'split.csv'))
...
np.savetxt('featurecr2019.csv', feature, delimiter = ',')