...
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_0.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]
        
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_1.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]
...
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_5.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_6.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]
        
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_7.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]
with open("/data2/whr/czl/TwiBot22-baselines/datasets/Twibot-22/tweet_8.json", 'r', encoding='UTF-8') as f:
    obj = ijson.items(f, "item")
    for x in tqdm(obj):
      if str(x['author_id']) in id_map.keys():
        text_user[x['text']]=id_map[str(x['author_id'])]

print(len(text_user.keys()))
