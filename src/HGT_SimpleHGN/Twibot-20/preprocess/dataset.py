...
def fast_merge(dataset="Twibot-20", server_id="209"):
    ...
    print(len(label), len(split))
    
    # assert len(label) == len(split)
    
    for i in range(length):
        if split["split"][i]!="support":
...
def hetero_graph_vectorize(include_node_feature=False, dataset="cresci-2015", server_id="209"):
    ...
    # print(tweet.head(100))
    user_text = [text for text in user.description]
    tweet_text = [text for text in tweet.text]
    
    print(len(user_text), len(user_index_to_uid))
    print(len(tweet_text), len(tweet_index_to_tid))
    ...
    if include_node_feature:
        ...
        for text in tqdm(tweet_text):
            ...
            tweet_text_feats.append(model(**encoded_input)["pooler_output"][0].cpu())
        
        for t in tqdm(user_text):
            if t is None:
                user_text_feats.append(torch.zeros(768))
                continue
            encoded_input = tokenizer(t, return_tensors="pt", max_length=512, truncation=True)
            encoded_input = encoded_input.to(device)
            user_text_feats.append(model(**encoded_input)["pooler_output"][0].cpu())
