...
with warnings.catch_warnings():
    import torch
    from model import GNN_classifier_3_layer_viz, GNN_classifier, GNN_classifier_1_layer, GNN_classifier_3_layer, GNN_classifier_4_layer, GNN_classifier_5_layer, GNN_classifier_6_layer, GNN_classifier_7_layer, GNN_classifier_8_layer
...
class homo_Trainer:
    def __init__(
        ...
        #     self.model = GNN_classifier_3_layer_viz(input_dim, 2, hidden_dim, activation)
    ...
    def valid(self, name="valid"):
        ...
        for batch in loader:
            ...
            out, embedding = self.model(batch.x, batch.edge_index)
            label = batch.y[:n_batch]
            out = out[:n_batch]
            embedding = embedding[:n_batch]
        ...
        # Save to /output/{self.config}_embedding.pt
        torch.save(all_label, f"./output/{self.config}_{name}_label.pt")
        torch.save(all_pred, f"./output/{self.config}_{name}_pred.pt")
        torch.save(all_embedding, f"./output/{self.config}_{name}_embedding.pt")
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            if epoch == self.epochs-1:
                ...
                torch.save(all_label, f"./output/{self.config}_train_label.pt")
                torch.save(all_pred, f"./output/{self.config}_train_pred.pt")
                torch.save(all_embedding, f"./output/{self.config}_train_embedding.pt")
