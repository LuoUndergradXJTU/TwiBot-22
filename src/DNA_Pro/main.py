...
if __name__ == '__main__':
    dataset_path = './tmp/datasets_trainset'
    ...
    for dataset_name in dataset_list:
        ...
        if '{}_LCS.txt'.format(dataset_name[:-8]) in os.listdir('./tmp/datasets_trainset/'):
            print('{} LCS exists.'.format(dataset_name[:-8]))
            f = open('./tmp/datasets_trainset/{}_LCS.txt'.format(dataset_name[:-8]))
            for line in f:
                try:
        ...
        w = open('./tmp/datasets_trainset/{}_LCS.txt'.format(dataset_name[:-8]), 'w')
        for item_id, item in enumerate(lcs_string):
            w.write('{} {}\n'.format(str(item_id), item))
        w.close()

        max_data = [-1, -1]  # Max MCC & Position
        for i in range(1, len(lcs_string) - 1):
            bot_sub_string = lcs_string[i]
            ...
            if matthews_corrcoef(y, pred) > max_data[0]:
                max_data[0] = matthews_corrcoef(y, pred)
                max_data[1] = i

        prep = max_data[1] / (len(lcs_string) - 1)

        f = open('./tmp/datasets_testset/{}'.format(dataset_name))
        for line in f:
            if len(line.split()) == 2:
                user_list[line.split()[0]] = line.split()[1]
            else:
        ...
        if '{}_LCS.txt'.format(dataset_name[:-8]) in os.listdir('./tmp/datasets_testset/'):
            ...
            raise Exception('Code error!', dataset_name)
